...
library(evir)
data(siemens)
SieDates <- as.character(format(as.POSIXct(attr(siemens, 
                                                "times")), "%Y-%m-%d"))
...
seriesPlot(SieRet, title = FALSE, main = "daily returns of siemens"
           , col = "blue")
boxPlot(SieRet, title = FALSE, main = "box plot returns siemens", 
        col = "blue", cex = 0.5, pch = 19)
acf(SieRet, main = "ACF of returns", lag.max = 20, ylab = "", xlab = "", 
    col = "blue", ci.col = "red")
pacf(SieRet, main = "PACF of returns",  lag.max = 20,  ylab = "", xlab = "", 
     col = "blue", ci.col = "red")
