...
lines(seq(along = p), ghd.VaR, col = "blue")
lines(seq(along = p), ghd.VaR, col = "green")
lines(seq(along = p), ghd.VaR, col = "orange")
legend("topright", 
...
plot(emp.ES, type = "l", xlab = "", ylab = "ES", axes = FALSE,
     ylim = range(c(hyp.ES, nig.ES, ghd.ES, nig.ES, nor.ES)))
box()
